    ...
    def step(self, actions):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if agent.done == 0:
                ...
                a += delta_theta
                a %= 360
                # update agent theta, px, py, vx, vy
        ...
        # compute reward
        reward, c_v = self.reward_callback(self.world, collision_mat, Reach_Goal, Exit_Boundary)
    ...
    def render(self, mode='human', output_file=None):
        ...
        if mode == 'human':
            ...
            goal = mlines.Line2D(goal_x, goal_y, color=goal_color, marker='*', linestyle='None', markersize=15,
                                 label='Goal')
            start = mlines.Line2D(start_x, start_y, color=start_color, marker='D', linestyle='None', markersize=15,
                                  label='Goal')
            ...
            # add agent and its goal
            agents_positions = [[self.states[i][j].position for j in range(self.agent_num)]
                                for i in range(len(self.states))]
            agents = [plt.Circle(agents_positions[0][i], self.agents[i].radius, fill=False) for i in range(self.agent_num)]
            ...
            goal = mlines.Line2D(goal_x, goal_y, color=goal_color, marker='*', linestyle='None', markersize=15,
                                 label='Goal')
            start = mlines.Line2D(start_x, start_y, color=start_color, marker='o', linestyle='None', markersize=12,
                                  label='Goal')
            # conflict_num = plt.text(-boundary, boundary, 'conflict_num %d' % (self.conflict_num_episode), color='red', fontsize=18)
            label = plt.text(boundary, boundary, 'GRL', color='blue', fontsize=18)
            for agent in agents:
            ...
            for i in range(self.agent_num):
                ...
                for state in self.states:
                    ...
                    orientation.append((  (agent_state.px, agent_state.py), (agent_state.px + r * np.cos(theta),
                                                                           agent_state.py + r * np.sin(theta))  ))
            ...
            def update(frame_num):
                ...
                for i in range(self.agent_num):
                    ...
                    if global_step % 4 == 0:
                        agent = plt.Circle(agents_positions[global_step][i], self.agents[i].radius, fill=False, color=cmap(i % 10))
                        ax.add_artist(agent)
                    for arrow in arrows:
                        ax.add_artist(arrow)

            anim = animation.FuncAnimation(fig, update, frames=len(self.states), interval=self.world.dt * 1000)
            anim.running = True

            if output_file is not None:
                # ffmpeg_writer = animation.writers['ffmpeg']
                # writer = ffmpeg_writer(fps=8, metadata=dict(artist='Me'), bitrate=1800)
                anim.save(output_file, writer='imagemagic', fps=10)
...
class MultiAgentEnv_maddpg(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        # compute reward
        reward, c_v, reward_ = self.reward_callback(self.world, collision_mat, Reach_Goal, Exit_Boundary)
        self.collision_value.append(c_v)
        ...
        self.simulation_done = terminal
        info = {'current_time_reach_goals': Reach_Goal, 'current_time_exit_boundary': Exit_Boundary, 'simulation_done': terminal, 'reward_eval': reward_}
...
# environment for all agents in the multiagent world of GRL
class MultiAgentEnv_DQN(MultiAgentEnv):
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 observation_callback=None, info_callback=None,
                 done_callback=None, shared_viewer=True):
        super(MultiAgentEnv, self).__init__()
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            start = mlines.Line2D(start_x, start_y, color=start_color, marker='D', linestyle='None', markersize=15,
                                  label='Goal')
            conflict_num = plt.text(-boundary, boundary, 'conflict_num %d' % (self.conflict_num_episode), color='red', fontsize=18)
            label = plt.text(boundary, boundary, 'DQN', color='blue', fontsize=18)
            ax.add_artist(goal)
            ax.add_artist(start)
            ax.add_artist(conflict_num)
            ax.add_artist(label)
